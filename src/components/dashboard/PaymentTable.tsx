            ...
            {payments.length === 0 && !isLoading ? (
              ...
              payments.map((payment) => (
                <TableRow key={payment.id} className="border-border/30 hover:bg-secondary/30 transition-colors group">
                  ...
                  <TableCell className={`text-right font-semibold ${Number(payment.amount) < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                    {Number(payment.amount) > 0 ? '+' : ''}{formatCurrency(payment.amount)}
                  </TableCell>
